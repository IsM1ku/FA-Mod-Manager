...
class FAModManager(TkinterDnD.Tk):
    def __init__(self):
        ...
        tk.Button(top_frame, text="Settings", command=self.open_settings).pack(
            side="right", padx=5
        )
        tk.Button(top_frame, text="Music Manager", command=self.open_music_manager).pack(
            side="right", padx=5
        )
        ...
        self.profile_list = ttk.Treeview(
            profiles_frame, show="tree", selectmode="browse", height=10
        )
        ...
        for game_key, profile in backend.list_existing_profiles():
            ...
            self.profile_smallfs[profile] = (
                game_key,
                backend.get_profile_smallf(game_key, profile),
            )
        self.update_profile_placeholder()
        tk.Button(
            profiles_frame, text="Set Active", command=self.set_active_profile
        ).pack(fill="x", pady=2)
        tk.Button(
            profiles_frame, text="Revert to Original", command=self.restore_original
        ).pack(fill="x", pady=2)
        tk.Button(
            profiles_frame, text="Import Profile", command=self.import_profile
        ).pack(fill="x", pady=2)
        b_frame = tk.Frame(profiles_frame)
        b_frame.pack()
        tk.Button(b_frame, text="Rename", command=self.rename_profile).pack(
            side="left", padx=2
        )
        tk.Button(b_frame, text="Delete", command=self.delete_profile).pack(
            side="left", padx=2
        )
        ...
        scrollbar = tk.Scrollbar(
            canvas_frame, orient="vertical", command=self.mods_canvas.yview
        )
        ...
        tk.Button(btn_frame, text="Up", width=6, command=self.move_up).pack(
            side="left", padx=2
        )
        tk.Button(btn_frame, text="Down", width=6, command=self.move_down).pack(
            side="left", padx=2
        )
        tk.Button(btn_frame, text="Clear", width=6, command=self.clear_mods).pack(
            side="left", padx=2
        )
        tk.Button(btn_frame, text="Add Files", command=self.add_files).pack(
            side="left", padx=2
        )
        tk.Button(btn_frame, text="Enable All", command=self.enable_all).pack(
            side="left", padx=2
        )
        tk.Button(btn_frame, text="Disable All", command=self.disable_all).pack(
            side="left", padx=2
        )
        tk.Button(mods_frame, text="Merge", command=self.merge_mods).pack(
            pady=4, fill="x"
        )
        ...
        self.info_name = tk.Label(
            info_frame, text="", font=("TkDefaultFont", 10, "bold")
        )
        ...
        tk.Button(
            info_frame, text="Remove Mod", command=self.remove_selected_mods
        ).pack(pady=5)
    ...
    def open_settings(self):
        ...
        # --- ISO selection ---
        tk.Label(win, text="Xbox 360 ISO").grid(
        ...
        tk.Button(win, text="Browse", command=browse_iso).grid(
            row=row, column=2, padx=5
        )
        row += 1

        tk.Label(win, text="Extract to").grid(
            row=row, column=0, sticky="w", padx=5, pady=2
        )
        extract_var = tk.StringVar(value=self.extract_root)
        tk.Entry(win, textvariable=extract_var, width=40).grid(
        ...
        tk.Button(win, text="Browse", command=browse_extract).grid(
            row=row, column=2, padx=5
        )
        row += 1
            ...
            try:
                ...
                messagebox.showerror(
                    ...
                    (
                        ...
                        "Check that the ISO path is correct and you have"
                        " permission to write to the destination folder."
                    ),
                )

        tk.Button(win, text="Extract ISO", command=run_extract).grid(
        ...
        tk.Checkbutton(win, text="Enable logging", variable=log_var).grid(
            row=row, column=0, columnspan=3, sticky="w", pady=4, padx=5
        )
        row += 1
        comment_var = tk.BooleanVar(value=self.comments_enabled)
        tk.Checkbutton(
            win, text="Add mod comments (experimental)", variable=comment_var
        ).grid(row=row, column=0, columnspan=3, sticky="w", pady=2, padx=5)
        row += 1
    ...
    def open_music_manager(self):
        game_root = self.game_paths.get("Full Auto 2: Battlelines (PS3)", "")
        if not game_root:
            messagebox.showerror(
                "Game Folder Missing",
    ...
    def set_active_profile(self):
        ...
        if not game_root:
            messagebox.showerror(
                "Error", "Game folder not set. Please use Settings first."
            )
            return
        ...
        try:
            ...
            messagebox.showinfo(
                "Active Profile",
                f"Copied modified smallf for '{profile}' to game folder.",
            )
        except Exception as exc:
            messagebox.showerror(
                "Export Error",
                (
                    f"Failed to copy files to the game folder:\n{exc}\n\n"
                    "Verify the game path and close any programs that might"
                    " be using these files."
                ),
            )
    ...
    def rename_profile(self):
        ...
        if selected:
            ...
            new_name = simpledialog.askstring(
                "Rename Profile", f"Enter new name for '{old_name}':"
            )
            if new_name and new_name != old_name:
                try:
                    ...
                    if game_key is None:
                        game_key = (
                            "fa2" if "Full Auto 2" in self.game_var.get() else "fa"
                        )
                    ...
                    messagebox.showerror(
                        "Rename Error",
                        (
                            f"Could not rename profile:\n{exc}\n\n"
                            "Ensure the new name is unique and try again."
                        ),
                    )
                    return
                ...
                self.profile_list.insert(
                    "", selected[0], iid=new_name, text=new_name, image=icon
                )
            ...
            try:
                ...
                messagebox.showerror(
                    ...
                    (
                        ...
                        "Ensure the profile is not open in another program."
                    ),
                )
                return
            self.profile_list.delete(selected[0])
    ...
    def restore_original(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Restore Error",
                (
                    f"Could not restore original files:\n{exc}\n\n"
                    "Ensure the game folder is correct and that files are"
                    " writable."
                ),
            )
    ...
    def import_profile(self):
        folder = filedialog.askdirectory(
            title="Select folder with smallf.dat", mustexist=True
        )
        if not folder:
            return
        ...
        if not smallf_path:
            messagebox.showerror(
                "Error",
                "smallf.dat not found in the selected folder.\n"
                "Please pick the folder that contains the file.",
            )
            return
        ...
        existing = [
            self.profile_list.item(i, "text") for i in self.profile_list.get_children()
        ]
        if profile_name not in existing:
            icon = self.icon_fa2 if game_key == "fa2" else self.icon_fa
            self.profile_list.insert(
                "", "end", iid=profile_name, text=profile_name, image=icon
            )
        self.update_profile_placeholder()
        ...
        try:
            ...
            messagebox.showerror(
                ...
                (
                    ...
                    "Check the file format or try another mod file."
                ),
            )
            return
    ...
    def select_mod(self, index):
        ...
        if self.selected_mod_index is not None and 0 <= self.selected_mod_index < len(
            self.mod_entries
        ):
    ...
    def move_up(self):
        ...
        self.mod_entries[idx - 1], self.mod_entries[idx] = (
            self.mod_entries[idx],
            self.mod_entries[idx - 1],
        )
    ...
    def show_mod_info(self, index):
        ...
        if game_key not in entry["games"] and "both" not in entry["games"]:
            self.info_warning.configure(
                text="This mod is not compatible with the selected game."
            )
        else:
    ...
    def merge_mods(self):
        ...
        try:
            ...
            existing = [
                ...
                for i in self.profile_list.get_children()
            ]
            if merge_name not in existing:
                icon = self.icon_fa2 if game_key == "fa2" else self.icon_fa
                self.profile_list.insert(
                    "", "end", iid=merge_name, text=merge_name, image=icon
                )
                self.update_profile_placeholder()
            messagebox.showinfo(
                "Merge Complete",
            ...
            messagebox.showerror(
                ...
                (
                    f"Mod merge failed:\n{exc}\n\n"
                    "Check that all selected mods are valid and try again."
                ),
            )
